...
export function getCache(key: string): any | null {
	try {
		...
		console.error(`Error reading database cache for key ${key}:`, error);
		return null;
	}
}
...
export function setCache(key: string, data: any, ttlSeconds: number): void {
	try {
		...
		console.error(`Error writing database cache for key ${key}:`, error);
	}
}
...
export function deleteCache(key: string): void {
	try {
		...
		console.error(`Error deleting database cache for key ${key}:`, error);
	}
}
...
function pruneCache(): void {
	try {
		...
		console.error('Error pruning database cache:', error);
	}
}
